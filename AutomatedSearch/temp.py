...
class Agency:
    ...
    @retry(wait=wait_random_exponential(min=1, max=10060), stop=stop_after_attempt(6))
    ...
    def demo_gradio(self, height=600):
        ...
        """
        # Wrap the demo in a try-except block to handle any errors
            ...
            with gr.Blocks() as demo:
                ...
                def user(user_message, history):
                    try:
                        ...
                        #logging.error("Error in user function: %s", e)
                        # Potentially notify for restart or take other actions
                        print(e)
                        user_message = "👤 User: " + user_message.strip()
                        return "", history + [[user_message, None]]
                ...
                def bot(history):
                    ...
                    try:
                        ...
                        user_message = "👤 User: Please continue where you left off be sure to inform others to save their work and check each others memory if they need something from another agent. "
                        return "", history + [[user_message, None]]
                    except Exception as e:
                        ...
                        user_message = "👤 User: Please continue where you left off be sure to inform others to save their work and check each others memory if they need something from another agent. "
                        return "", history + [[user_message, None]]
                        #raise  # Optionally re-raise to let Gradio handle it 
    ...
    def _create_send_message_tool(self, agent: Agent, recipient_agents: List[Agent]):
        ...
        class SendMessage(BaseTool):
            ...
            @staticmethod
            def get_corrected_recipient_name(incorrect_name: str) -> str:
                import instructor
                ...
                try:
                    system_message = f"Please correct the incorrect agent recipient name '{incorrect_name}' Vaild agent recipient names are: {recipient_names}"
                    user_message = "Corrected agent recipient name:"
            ...
            #    valid_caller_names = [agent.name for agent in outer_self.agents]
            #    if value not in valid_caller_names:
            #        raise ValueError(f"Caller agent name must be one of {valid_caller_names}.")
            #    return value

            def run(self):
                # Get correct recipient name
                self.recipient.value = self.get_corrected_recipient_name(self.recipient.value)
                thread = outer_self.agents_and_threads[agent.name][self.recipient.value]

                #gen = thread.get_completion(message=self.message)
                #gen = thread.get_completion(message=self.message + "\n" + "Pass this along: Check your memory. Save your work.")
                gen = thread.get_completion(message=self.message + "\n" + "ExtendedThoughtProcessTool MUST be used before sending messages. Check relevant memory. Save your work.")
                time.sleep(wait_time)
                try:
                    while True:
                        yield next(gen)
